/* Service worker — makes offline deterministic rather than leaving it to the
   ...
   no server data to go stale, so a cached hit is always correct. Updates are
   picked up in the background and applied on the next launch.

   Bump CACHE when the app changes; the build script does that automatically. */
const CACHE = "tb20-v2026.08.09.1305";
